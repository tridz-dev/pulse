...
def pulse_employee_conditions(user: str | None = None, doctype: str | None = None) -> str:
	"""Restrict Pulse Employee visibility by role: own only for Operator, subtree for others."""
	user = user or frappe.session.user
	if frappe.session.user == "Administrator" or "Pulse Admin" in frappe.get_roles(user):
		return ""
	...
	if "Pulse Leader" in roles:
		...
		return f" `tabPulse Employee`.name in ({','.join(frappe.db.escape(e) for e in subtree)}) "
	if "Pulse Manager" in roles:
		subs = _get_subordinate_employee_names(emp)
		subs.append(emp)
